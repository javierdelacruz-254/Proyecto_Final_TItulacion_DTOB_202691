  ...
  Widget build(BuildContext context) {
    final filtrados = centros.where((centro) {
      final coincideDistrito =
          filtroDistrito.isEmpty ||
          centro.distrito.toLowerCase().contains(filtroDistrito.toLowerCase());

      final coincideTipo = filtroTipo.isEmpty || centro.tipo == filtroTipo;

      final coincideServicio =
          filtroServicio.isEmpty || centro.servicios.contains(filtroServicio);

      return coincideDistrito && coincideTipo && coincideServicio;
    }).toList();
